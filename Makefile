...
# The header slate, without the leading hash: an unescaped '#' in a make
...
og: ## Re-render og.png from tools/og.html
	$(CHROME) --headless=new --disable-gpu --hide-scrollbars \
		--force-device-scale-factor=1 --default-background-color=$(SLATE)FF \
		--screenshot=/tmp/ble-og-raw.png --window-size=1200,630 tools/og.html
	$(MAGICK) /tmp/ble-og-raw.png -strip -colors 64 PNG8:og.png
	@rm -f /tmp/ble-og-raw.png
	@echo "wrote og.png"

.PHONY: favicon
favicon: ## Re-render favicon.ico and apple-touch-icon.png from the SVG sources
	$(MAGICK) -background "#$(SLATE)" favicon.svg -resize 180x180 \
		-alpha off -colorspace sRGB -type TrueColor -strip apple-touch-icon.png
	$(MAGICK) \
		\( -background "#$(SLATE)" favicon.svg -resize 48x48 \) \
		\( -background "#$(SLATE)" favicon.svg -resize 32x32 \) \
		\( -background "#$(SLATE)" tools/favicon-16.svg -resize 16x16 \) \
